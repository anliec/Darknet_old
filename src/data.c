...
void correct_boxes_angle(box_label *boxes, int n, float dx, float dy, float sx, float sy, int flip, float sinA, float cosA, image orig) {
    ...
    for (i = 0; i < n; ++i) {
        ...
        // apply rotation
        float new_x = (boxes[i].x * cosA + boxes[i].y * sinA);
        ...
        boxes[i].top    = boxes[i].y - (boxes[i].h / 2);
        boxes[i].bottom = boxes[i].y + (boxes[i].h / 2);
...
//        const float angle = 0.f;
        ...
        const float dw = jitter * w;
        const float dh = jitter * h;
        const float new_ar = (w + rand_uniform(-dw, dw)) / (h + rand_uniform(-dh, dh));
        float nw, nh;
        if (new_ar < net_ar) {
            nh = h;
            nw = nh * new_ar;
        ...
        float not_rotated_w = (nw * cosA + nh * aSinA);
        float not_rotated_h = (nh * cosA + nw * aSinA);
        const float max_possible_scale_h = orig.h / not_rotated_h;
        const float max_possible_scale_w = orig.w / not_rotated_w;
